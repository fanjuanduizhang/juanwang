...
public class AnswerServiceImpl extends ServiceImpl<AnswerMapper, Answer> implements AnswerService {
    ...
    @Override
    public void updateManualScore(ManualScoreRequest request) {
        if (request.getAnswerId() == null) {
            throw new InternalServerError("答案ID不能为空");
        }
        if (request.getQuestionId() == null) {
            throw new InternalServerError("题目ID不能为空");
        }
        if (request.getScore() == null) {
            throw new InternalServerError("分数不能为空");
        }

        Answer answer = getById(request.getAnswerId());
        if (answer == null) {
            throw new InternalServerError("答案不存在");
        }

        LinkedHashMap<String, Object> answerMap = answer.getAnswer();
        if (answerMap == null) {
            answerMap = new LinkedHashMap<>();
        }

        Map<String, Object> questionAnswer = (Map<String, Object>) answerMap.get(request.getQuestionId());
        if (questionAnswer == null) {
            questionAnswer = new LinkedHashMap<>();
        }
        questionAnswer.put("examScore", Collections.singletonMap("examScore", request.getScore()));
        answerMap.put(request.getQuestionId(), questionAnswer);
        answer.setAnswer(answerMap);

        Project project = projectMapper.selectById(answer.getProjectId());
        if (project != null) {
            ProjectView projectView = projectService.getProject(answer.getProjectId());
            computeExamScore(answer, projectView);
        }
