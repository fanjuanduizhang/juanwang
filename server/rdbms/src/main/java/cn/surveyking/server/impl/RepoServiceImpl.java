...
public class RepoServiceImpl extends BaseService<RepoMapper, Repo> implements RepoService {
	...
	@Override
	public void copyFromRepos(CopyFromReposRequest request) {
		String currentUserId = SecurityContextUtils.getUserId();
		List<String> sourceRepoIds = request.getSourceRepoIds();

		// 如果未指定源题库，则获取用户有权限的全部题库（自己创建的 + 共享的）
		if (sourceRepoIds == null || sourceRepoIds.isEmpty()) {
			List<Repo> allRepos = list(Wrappers.<Repo>lambdaQuery()
					.eq(Repo::getCreateBy, currentUserId)
					.or(x -> x.eq(Repo::getShared, 1)));
			sourceRepoIds = allRepos.stream().map(Repo::getId).collect(Collectors.toList());
		}

		List<Template> templatesToCopy = new ArrayList<>();
		List<Tag> tagsToCopy = new ArrayList<>();

		for (String sourceRepoId : sourceRepoIds) {
			// 构建查询条件
			var query = Wrappers.<Template>lambdaQuery()
					.eq(Template::getRepoId, sourceRepoId);

			// 按题型过滤
			if (request.getQuestionTypes() != null && !request.getQuestionTypes().isEmpty()) {
				query.in(Template::getQuestionType,
						request.getQuestionTypes().stream()
								.map(t -> SurveySchema.QuestionType.valueOf(t))
								.collect(Collectors.toList()));
			}

			// 按标签过滤（通过 t_tag 表关联）
			if (request.getTags() != null && !request.getTags().isEmpty()) {
				List<String> tagTemplateIds = tagService.list(Wrappers.<Tag>lambdaQuery()
						.in(Tag::getName, request.getTags())
						.eq(Tag::getCategory, TagCategoryEnum.template.name()))
						.stream().map(Tag::getEntityId).collect(Collectors.toList());
				if (!tagTemplateIds.isEmpty()) {
					query.in(Template::getId, tagTemplateIds);
				} else {
					// 没有匹配的标签，跳过该题库
					continue;
				}
			...
			List<Template> sourceTemplates = templateService.list(query);

			// 限制每个题库选取数量
			if (request.getLimitPerRepo() != null && sourceTemplates.size() > request.getLimitPerRepo()) {
				// 随机选取
				Collections.shuffle(sourceTemplates);
				sourceTemplates = sourceTemplates.subList(0, request.getLimitPerRepo());
			}

			for (Template source : sourceTemplates) {
				// 创建新题目，生成新ID
				Template newTemplate = new Template();
				String newId = IdWorker.getIdStr();
				newTemplate.setId(newId);
				newTemplate.setRepoId(request.getTargetRepoId());
				newTemplate.setSerialNo(source.getSerialNo());
				newTemplate.setName(source.getName());
				newTemplate.setQuestionType(source.getQuestionType());
				newTemplate.setTemplate(source.getTemplate()); // SurveySchema 对象，直接复用
				newTemplate.setMode(source.getMode());
				newTemplate.setCategory(source.getCategory());
				newTemplate.setPreviewUrl(source.getPreviewUrl());
				newTemplate.setPriority(source.getPriority());
				newTemplate.setShared(0);

				// 复制标签
				if (source.getTag() != null && source.getTag().length > 0) {
					newTemplate.setTag(source.getTag());
					for (String tagName : source.getTag()) {
						Tag tag = new Tag();
						tag.setName(tagName);
						tag.setEntityId(newId);
						tag.setCategory(TagCategoryEnum.template.name());
						tagsToCopy.add(tag);
					}
				}

				templatesToCopy.add(newTemplate);
			}
		}

		if (!templatesToCopy.isEmpty()) {
			templateService.saveBatch(templatesToCopy);
		}
		if (!tagsToCopy.isEmpty()) {
			tagService.saveBatch(tagsToCopy);
		}
	}
