...
public class AnswerScoreEvaluator {
    ...
    public AnswerScoreEvaluator(SurveySchema schema, LinkedHashMap answer) {
        ...
        evaluatorList.add(new ManualScoreEvaluator());
    }
    ...
    class ManualScoreEvaluator implements Evaluator {
        ...
        public Double eval(SurveySchema qSchema) {
            ...
            Object examScoreObj = qValue.get("examScore");
            if (examScoreObj == null) {
                return 0.0;
            }
            if (examScoreObj instanceof Map) {
                Map examScoreMap = (Map) examScoreObj;
                Object score = examScoreMap.get("examScore");
                if (score instanceof Number) {
                    return ((Number) score).doubleValue();
                }
            } else if (examScoreObj instanceof Number) {
                return ((Number) examScoreObj).doubleValue();
            }
            return 0.0;
        }
        ...
        public boolean support(SurveySchema qSchema) {
            ...
            return SurveySchema.ExamScoreMode.manual.equals(qSchema.getAttribute().getExamAnswerMode())
                    && answer.get(qSchema.getId()) != null;
        }
